...
public class Test_TimedUntil {
	...
	@Test
	public void test_until_rwithin() { // relative-time until
			
			var tr = genHopeFearTrace(data_("hope",0,0,0,1,2),
					                  data_("fear",0,0,0,-1,-1)) ;
			
			assertEquals(SATVerdict.UNSAT, tr.satisfy(until_rwithin(now(S -> S.hope() <= 0) , H(), null , 20f))) ;
			assertEquals(SATVerdict.SAT, tr.satisfy(until_rwithin(now(S -> S.hope() <= 0) , H(), null , 30f))) ;
			assertEquals(SATVerdict.SAT, tr.satisfy(until_rwithin(now(S -> S.hope() <= 0) , H(), null , 40f))) ;

			assertEquals(SATVerdict.UNSAT, 
					tr.satisfy(next(until_rwithin(now(S -> S.hope() <= 0) , H(), null , 10f)))) ;
			assertEquals(SATVerdict.SAT, 
					tr.satisfy(next(until_rwithin(now(S -> S.hope() <= 0) , H(), null , 20f)))) ;

			assertEquals(SATVerdict.SAT, tr.satisfy(until_rwithin(now(S -> S.hope() <= 0) , H(), 30f , 30f))) ;
			assertEquals(SATVerdict.UNSAT, tr.satisfy(until_rwithin(now(S -> S.hope() <= 0) , H(), 40f , null))) ;

			assertEquals(SATVerdict.SAT, 
					tr.satisfy(next(until_rwithin(now(S -> S.hope() <= 0) , H(), 20f , 20f)))) ;
			assertEquals(SATVerdict.UNSAT, 
					tr.satisfy(next(until_rwithin(now(S -> S.hope() <= 0) , H(), 30f , null)))) ;

			assertEquals(SATVerdict.UNSAT, 
					tr.satisfy(next(next(until_rwithin(now(S -> S.hope() <= 0) , H(), 20f , 20f))))) ;
		}
